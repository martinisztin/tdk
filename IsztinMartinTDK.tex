...
Számos kutatás számol be a feltörekvõ technológia automatikus programjavító képességeirõl, viszont az ezt próbára tevõ egységtesztek generálásáról kevés információt tudunk.
...
További elõnyei az adathalmaznak, hogy a biztosított környezettel könnyedén lehet alternálni a projekt sérülékeny és javított változatai között, 
...
A kutatás során az egyik legismertebbek nyelvi modellt használtuk, az OpenAI cég generatív elõtanított transzformer modelljét, a GPT-t, ezen belül is a legfrissebb GPT-4 Turbo változatát.
A bemenetként adott szöveghez (prompthoz) kizárólag egy szerepet 
(ami lehetõvé teszi az LLM számára, hogy pontosabb, kontextusnak megfelelõ összhangban álló válaszokat adjon, itt most a modell egy senior szoftvertesztelõ) 
és a rendelkezésre álló kódot adtuk át. 
Az eredmények osztályozása két részre különíthetõ. Elõször egy automatizált környezetben végeztük el a kontextus legyártását, promptolást, a kapott válasz feldolgozását és a kiértékelését, ezt követõen kézzel is 
felülbíráltuk a generált kódok szubjektív használhatóságát.

A kutatás a következõ kérdésekre keresi a választ:

\begin{itemize}
\item \textbf{RQ1}: Hány százalékban kaptunk szintaktikailag helyes kódot?
\item \textbf{RQ2}: Hány százalékban kaptunk szemantikailag is helyes kódot?
\item \textbf{RQ3}: Milyen a szubjektív használhatósága a generált teszteknek?
\item \textbf{RQ4}: Hogyan befolyásolja a kontextus a generálás pontosságát?
\end{itemize}
...
31\%-ban állítás (assertion) nélküli tesztet generál és az esetek 37\%-ában nem hívja meg a tesztelendõ metódust. A megerõsítéses tanítás eredményeként felügyelt finomhangolással és az RLSQM 
...
A használt modell tanítása progresszíven történt. Történt egy angol nyelvû elõtanítás, utána egy kódbázisú, végül egy teszteset generáláshoz kapcsolódó finomhangolás.
...
A fejlesztõk leírnak néhány peremfeltételt vagy követelményeket és a modell kimenete releváns tesztesetek lesznek, amik kiértékelhetõk a rendszer megfelelõ végrehajtási útvonalaihoz.
...
Habár mi is teszt eseteket generálunk, a fentiekkel szemben a mi kutatásunk nem csak egy adott funkcionalitás tesztelését célozza a nagyobb lefedettség érdekében, 
...
\begin{figure}[!hbp]
  ...
  \centering
  \includegraphics[width=0.45\textwidth,natwidth=1195,natheight=1850]{graph/martin_method.pdf}
  \caption{A módszertan ábrázolása}
\end{figure}
...
Igyekeztünk minél optimálisabb és lényegre törõbb bemeneti szöveget definiálni, ezért a kísérleti futtatásokból kinyert tapasztalatok alapján formáltuk meg a végleges promptot.
...
amely szintaktikailag is helyesen strukturálva tartalmazta a szintnek megfelelõ mennyiségû adatot az adott sérülékenységrõl.
...
A kézi kiértékelés során több fejlesztõ is dolgozott a validáción, az emberi hibák csökkentésének érdekében.
...
A szintaktikai elemzés során az volt a kérdés, hogy a GPT képes-e mindenféle emberi beavatkozás nélkül olyan egységtesztet generálni, 
amely univerzálisan alkalmazható egy tesztkörnyezetben, feltéve, hogy a modellnek az utasítás részeként egyszerû Java nyelvi elemek használatát kértük a válaszában.
Elõfordultak olyan esetek, ahol a futtató környezet extra jelölést igényelt a teszt lefuttatása érdekében. 
Ezen kontextus hiánya miatt voltak olyan generálások, ahol a tesztet a futtató környezet nem érzékelte egységtesztként, így az nem futott le, de a projekt fordítási folyamatába beletartozott.
...
a példában szereplõ javítás egy végtelen ciklus kiküszöbölését tartalmazza, ellenben a teszt egy \textit{long} típusú számértéket \textit{integerre} való típuskasztolás segítségével akar összehasonlítani egy másik integerrel.
Azonban ezek sem véletlenszerû utasítások, hiszen relevánsak a promptban specifikált kódhoz, viszont a sérülékenység teszteléséhez nem tesznek eleget.%, így a manuális kiértékelés során ezt az esetet nem ítéltük hasznosnak.

\begin{lstlisting}[caption={Az L1 VUL4J-12 példára generált teszteset részlete}, captionpos=b, breaklines=true]
  @Test
  public void testExtendMethodVulnerability() throws Exception {
      ...
      assertEquals("The extend method should behave as expected after the patch", expectedInt, result);
  }
\end{lstlisting}

\begin{lstlisting}[caption={A VUL4J-12 példához tartozó javítás},label={lst:vul4j7}, captionpos=b, keepspaces=true, language=diff]
  ...
  int vt = (1 << (t - 1));
-  while (v < vt) {
+  if (v < vt) {
...
Másik eset egy hasznos generálásra, az \textbf{L2}-ben szereplõ \textit{VUL4J-52} példához készült teszt.
A kódban fennálló \textit{helytelen jogosultságkezelés} sérülékenység javítása érdekében implementáltak egy metódust, 
ami a sandbox környezetbe bekerülõ kódban megtiltja a \textit{finalize} függvény felülírását egy \textit{SecurityException} kivételdobás kíséretében.

\begin{lstlisting}[caption={Az L2 VUL4J-52 példára generált teszteset}, captionpos=b, breaklines=true]
  @Test
  public void testForbidIfFinalizer_withUnsafeFinalizeMethod_ShouldThrowSecurityException() {
...
A generált teszt valóban leteszteli azt az eshetõséget, hogy a környezetbe egy ilyen függvény kerül, és el is várja a megfelelõ kivétel keletkezését, ami 
a sérülékeny kódban nem fog megtörténni, ellenben a javítottal.

\begin{framed}
  \noindent \textbf{RQ3: Milyen a szubjektív használhatósága a generált teszteknek?}
...
\begin{framed}
  \noindent \textbf{RQ4: Hogyan befolyásolja a kontextus a generálás pontosságát?}
  ...
  11 esetben (55\%) ítéltük szemantikailag hasznosnak a tesztet, \textbf{L1} esetén 12 esetben (60\%), \textbf{L2} esetén 15 (75\%), \textbf{L3}-ban pedig 13 alkalommal (65\%).
...
\section{Visszakérdezéssel kapcsolatos tapasztalatok}
\label{sec:misc}
...
A prompt szerkezetileg jól strukturált volt, ám nem tartalmazott extra adatot és információt a javítás elõsegítéséhez.
...
Kutatásunk bemutatja a GPT-4 egységteszt generáló képességeit a sérülékenységek kontextusában, valós környezetben.
...
\begin{enumerate}
  ...
  \item A visszakérdezések, amelyek elegendõ információt tartalmaznak a hiba kijavításához, inkább a helyes irányba terelik a generálást, mint a rossz irányba.
\end{enumerate}

Összegzésként, a nagy nyelvi modellek által történõ egységteszt generálás egyáltalán nem haszontalan irány, 
azonban a teljesítmény javításához további kutatások szükségesek (például az ~\ref{chap:lim}. fejezetben felsorolt célok megvalósításával).
